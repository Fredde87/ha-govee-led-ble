...
def parse_color_mode(generated: Any, model: str) -> ParsedColorModeResponse:
    ...
    if mode_name == "video" and get_profile(model).supports_video_mode:
        # Gated on the PROFILE, not on the grammar.  Mode byte 0x00 means video on a tvlightv4
        # device and nothing on an H617A, and both read the same status grammar -- so the
        # grammar can name the case while only a model that has video may take this branch.
