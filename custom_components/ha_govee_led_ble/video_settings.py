...
# clean undo. A read cannot do that, so reads are allowed where the APK proves the register
# has a get form; see CALIBRATION_READ_ALLOWLIST below.
VIDEO_SETTING_READS: tuple[int, ...] = (0x01, 0x04, 0x06, 0x09, 0x0A, 0x0B, 0x10, 0x11, 0x13)

# Calibration registers whose READ form the APK proves is a getter rather than a trigger.
...
# Two bytes, and the APK names both. **from APK** -- VideoHdrEffectController is symmetric,
# which is what makes this a reading rather than a guess:
#
...
# not the explanation.
#
# **from APK.** `Controller4Hdmi` (0x03) and `VideoSaturationController` (0x0e) are each
# constructed in exactly one place, `pact_h605b.newdetailhdmi.viewmodel.NewDetailHdmiVm`,
# and the base video sheet's `AbsVideoNewDetailVm.supportSaturation()` returns false --
# only the HDMI view model overrides it. Which screen a device gets is
# `Support.isHdmiSku(goodsType)`, goodsType in {138, 142, 192, 243} = H6601..H6604.
#
# That gate is NOT reachable from here: goodsType is a cloud field, not pactType/pactCode,
# and there is no capability query for it. So these stay named and unavailable rather than
# deleted -- a sync-box owner should get them, an H66A0 owner should not see a dead control.
# Full chain in COMMANDS.md 2.6.
...
# NOT WRITTEN by this pact and not identified. Reads 0x02 on the one device driven here and
