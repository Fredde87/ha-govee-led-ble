...
class _GoveeExtraServicesMixin(_GoveeLightOwner):
    ...
    async def async_set_video_settings(
        ...
        self._require_support(
            ...
            and not self.coordinator.profile.uses_h6199_video_body,
        )
    ...
    async def async_read_dreamview_group(self) -> dict[str, Any]:
        ...
        in them: the vendor app gets that from Govee's cloud
        (`/bff-app/v1/moment/feasts/sub-devices`, which needs an account token), not over BLE.
    ...
    async def async_set_dreamview_group(self, members: list[dict[str, Any]]) -> None:
        ...
        That failure could not be caught afterwards. `areaNum` is never reported over BLE (the
        vendor app reads it from `GET /bff-app/v1/moment/feasts/sub-devices`, which is account-
        gated), this upload has no read-back, and `delete_dreamview_group` is the only way out of
        a bad write. So the count is now demanded from the caller rather than guessed.
        """
        self._require_support(
            "set_dreamview_group",
            supported=self.coordinator.profile.supports_dreamview,
