...
``BgcInfoReader.g()`` in the vendor app treats encryption version 1 *or* 2 as "encrypted",
and only ``h()`` is v2-specific -- so v1 is not a curiosity, it is half of the devices the
app is willing to encrypt to. It went uninvestigated here until an H61F5 turned up with **no
2b12 characteristic at all**, which made it look unencrypted while silently ignoring every
plaintext frame sent to it.

Transcribed from ``com.govee.encryp.ble.Safe`` and ``com.govee.encryp.ble.Controller4Aes``.
...
# LibTools.c() -- KEY_COMMUNICATION, recovered from the app_communication string resource
# under app_session, exactly as K_HS and K_DEV are. App-global, not per-device, not a secret.
# The plaintext is ASCII "MakingLifeSmarte".
