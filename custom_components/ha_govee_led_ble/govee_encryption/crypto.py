...
The scheme is documented in full in PROTOCOL.md (linked from the pull request that
introduced this package). Short version: newer Govee devices wrap the classic 20-byte
command frame in AES-128-GCM. Both keys below are app-global constants compiled into the
Govee Home APK -- they are not secrets and not per-device. The per-device key is derived
from the SKU and BLE MAC, both of which the device itself broadcasts. No cloud call and
no ``secretCode`` are involved.
"""
...
KEY_DEVICE = bytes.fromhex("AE028B630BAE6ECC4BFF1B249E22F955")  # LibTools.b()
...
def parse_encryption_version(data: bytes | None) -> int:
    ...
    Two formats exist (com.govee.encryp.ble.BgcInfoReader.d())::
...
def parse_bgc_info_v2(data: bytes) -> tuple[int, int] | None:
    ...
    These are the app's own capability keys. Untested against hardware: the device we can
    test returns the format-1 blob, which does not carry them. The same pair is also
    readable with ``aa ef`` and from the BLE advertisement, both of which are verified.
    """
