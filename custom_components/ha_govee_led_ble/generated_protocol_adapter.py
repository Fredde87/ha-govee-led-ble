...
def build_relative_brightness_query(model: str) -> bytes:
    ...
    # Keyed on the VIDEO grammar, which is what owns this register: an H6199 has its own form,
    # and the H617A family carries the same domain behind a 0x01 selector.
    return _build_status_query("relative_brightness", _video_grammar(model))
...
def build_camera_install_query(model: str = "H617A") -> bytes:
...
def build_ic_segment_count_query(model: str = "H617A") -> bytes:
...
def build_video_mode_h66a0(
    *,
    ...
    Every field is named from the APK, verified in both directions against
    pact_tvlightv4's writer and parser, and corroborated by a labelled read-back; see
    command_write.ksy::video_body_h66a0. ``picture_preset`` is one of PICTURE_PRESETS and is
    encoded as ``0x08 | index``.

    ``reserved`` is a byte this pact never writes; pass back whatever the device reported so a
...
def build_video_setting_query(setting: int, model: str = "H617A") -> bytes:
    return _build_status_query("display_setting", get_profile(model).command_grammar, video_setting=setting)
