        ...
        async with self._control_lock:
            ...
            try:
                ...
                if client and client.is_connected:
                    query = build_video_setting_query(BLACK_SCREEN_DETECTION_SETTING)
    ...
    async def async_set_hdr_effect(self, enabled: bool, gear: int) -> None:
        ...
        async with self._control_lock:
            ...
            try:
                ...
                client = self._client
                if client and client.is_connected:
                    query = build_video_setting_query(HDR_EFFECT_SETTING)
                    self._record_packet("tx", query, outcome="sent", reason="write_succeeded")
                    await self._async_write_packet(client, query)
            except Exception:
                if previous is None:
    ...
    async def async_set_ai_filter(self, enabled: bool) -> None:
        ...
        async with self._control_lock:
            ...
            try:
                ...
                client = self._client
                if client and client.is_connected:
                    query = build_video_setting_query(AI_FILTER_SETTING)
                    self._record_packet("tx", query, outcome="sent", reason="write_succeeded")
                    await self._async_write_packet(client, query)
            except Exception:
    ...
    async def async_set_black_border_removal(self, enabled: bool) -> None:
        ...
        async with self._control_lock:
            ...
            try:
                ...
                client = self._client
                if client and client.is_connected:
                    query = build_video_setting_query(BLACK_BORDER_REMOVAL_SETTING)
                    self._record_packet("tx", query, outcome="sent", reason="write_succeeded")
                    await self._async_write_packet(client, query)
            except Exception:
                if previous is None:
