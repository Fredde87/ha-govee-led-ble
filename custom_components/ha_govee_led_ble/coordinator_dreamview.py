...
The hard limit that shapes everything here: the sync centre reports per-slot connection state
and per-member brightness, and NEVER an address. The vendor app gets addresses and names from
Govee's cloud (`/bff-app/v1/moment/feasts/sub-devices`, account-gated), which is also why the
Area Config is write-only on the wire. Without a token the only route is observation, which is
what :meth:`_DreamviewMixin.async_identify_dreamview_members` does.
"""
...
class _DreamviewMixin(_CoordinatorBase):
    ...
    async def async_read_dreamview_state(self, *, timeout: float = 2.0) -> DreamviewState:
        ...
        the group -- the sync centre never reports their addresses, and the vendor app only
        knows them because its cloud account does. So a group created in the app shows up here
        as a member count and its settings, and its members stay anonymous.
    ...
    async def async_identify_dreamview_members(self, *, settle: float = 30.0, poll: float = 1.0) -> dict[str, Any]:
        ...
        No BLE command returns it, and the reason is now known rather than inferred: the vendor
        app does not ask the device at all. It calls
        `GET /bff-app/v1/moment/feasts/sub-devices`, whose response carries `bleAddress`,
        `bleName`, `deviceName`, `areaConfigs`, `areaNum`, `cmdVer` and `goodsType` for every
        member -- the whole picture, from Govee's server. That endpoint is account-gated (it
        answers `401 authorization token is blank` without a token), and it is why `areaConfigs`
        is write-only on the wire: the cloud is the system of record, so the firmware never
        needed to report it back.

        Confirming the absence on the BLE side: the complete `0x60` sub-command space is switch,
        music mode, brightness, brightness-unite, sub-device connect, reset, clear, music
        sub-effect, saturation, get-colour, sound, digest and delete -- none reports an address
        -- and every `0xa3` multi-packet controller in the app is write-only.
