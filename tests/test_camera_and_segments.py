...
def test_a_calibration_register_is_read_only_from_the_allowlist():
    """Reads are allowed only where the APK proves the register has a get form.
    ...
    # 0x05, 0x0d and 0x14 are never read either: nothing in the APK gives them a get form.
...
async def test_the_first_camera_probe_is_sent_from_the_unknown_state(hass) -> None:
    """A never-probed camera must be probed.

    The keep-alive loop re-probed only when the verdict was already False -- the hot-plug watch --
    so a freshly set-up entry sat at None forever: no probe, no verdict, and the video registers
    never read because those are fetched once a camera is found.  Observed on hardware: an H66A0
    reported `camera_installed = None` and `video_settings = {}` indefinitely.
    """
    from custom_components.ha_govee_led_ble.const import ReadDomain, get_profile

    profile = get_profile("H66A0")
    assert profile.can_read(ReadDomain.CAMERA_INSTALL)
    # The guard the loop uses: it must admit the un-probed state, not only the known-absent one.
    for verdict, should_probe in ((None, True), (False, True), (True, False)):
        assert (verdict is not True) is should_probe, f"verdict {verdict!r} probe gate wrong"


async def test_the_camera_probe_actually_writes_its_frame(hass):
    """The probe must reach the device, not raise on the way out.

    `_probe_camera` and the display-setting reads called `self._write_frame`, a name that
    existed only as a TYPE_CHECKING protocol declaration -- so mypy was satisfied and every
    call raised AttributeError at runtime.  It is not caught by the `except BleakError`
    around the writes, so it escaped and killed the keep-alive task that had just started it:
    no probe, no verdict, and the video registers never read.  Observed on hardware as
    `camera_installed = None` and `video_settings = {}` that never changed.
    """
    coordinator = _coordinator(hass)
    device = GoveeDeviceDouble("H66A0")
    client = FakeGoveeClient(device)
    with patch_connection(client):
        await coordinator._ensure_connected()
        client.frames.clear()
        await coordinator._probe_camera(full=False)
