...
def coord(hass):
    ...
    # The real write helper: stubbing a name the coordinator does not have would let these
    # tests pass while every one of these paths raised AttributeError on a device.
...
async def test_reading_the_group_answers_two_questions_and_invents_no_third(coord):
    ...
    # what happened here: the handler had no branch for a 0x60 frame, so the read waited forever
    # on a device that had already answered.  Driving the real receive path is what catches that.
    def _answer(_packet: bytes) -> None:
        coord._notify_callback(None, bytearray(digest))
        coord._notify_callback(None, bytearray(members))

    coord.send_command = AsyncMock(side_effect=_answer)
