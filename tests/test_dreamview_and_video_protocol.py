...
_CAPTURED_VIDEO_FRAMES = [
    ...
    # sound-effect softness. An earlier revision had them the other way round, on an APK reading
    # that looked convincing and was wrong; the owner then found that changing softness in the
...
def test_segments_to_mask_bounds_on_the_device_not_the_wire_field():
    ...
    assert segments_to_mask([14], MODEL_PROFILES["H66A0"]) == 1 << 13

    with pytest.raises(ValueError, match="out of range 1..14"):
        segments_to_mask([15], MODEL_PROFILES["H66A0"])
    with pytest.raises(ValueError, match="out of range 1..14"):
        build_segment_brightness([15], 50, "H66A0", profile=MODEL_PROFILES["H66A0"])
    # Past the field width as well as past the device: still the device's count that is quoted,
    # because that is the smaller bound and the one the caller can act on.
    with pytest.raises(ValueError, match="out of range 1..14"):
        segments_to_mask([16], MODEL_PROFILES["H66A0"])
    # With no profile the wire field is the only bound available.
...
def test_hdr_effect_layout_matches_the_devices_own_reply():
    ...
    `VideoHdrEffectController` is symmetric in the APK -- it writes
    `{17, 2, enabled, level}` and parses `{bArr[2] == 1, bArr[3]}` -- which is what makes
    this a reading rather than a guess about two bytes.
    """
...
def test_h66a0_claims_every_music_mode_it_was_proven_to_accept():
    ...
    The set was found by sweeping every id in the APK registry rather than reasoning about which
...
def test_h66a0_claims_relative_brightness_after_the_round_trip():
    ...
    # The query itself needs the H66A0's status grammar registered, which is the one item still
    # open upstream; the capability claim is what this test is really about.
    assert proto.build_relative_brightness_query("H6199").hex().startswith("aaae01")
...
def test_group_entry_puts_the_address_on_the_wire_reversed():
    """The APK reads the address in display order, then appends it last byte first.
...
def test_ai_filter_preserves_the_selected_filter():
    ...
    The app builds them from a definition fetched from `/bff-app/v1/filter/init`, which this
    integration has no access to. Zeroing them would clear a selection we cannot reconstruct, so
    a writer must hand back what it read.
    """
