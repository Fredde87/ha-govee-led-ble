...
@pytest.mark.parametrize(
    ...
    [
        ...
        (bytes.fromhex("020201000a03"), 2),  # format 2, from the APK
...
def _reconnect(session, device) -> None:
    """What a disconnect and reconnect does to both sides.

    A v1 session key is negotiated per connection, so the device draws a new one and answers
    the handshake again. V1Device answers only while it holds no key, which is right for one
    connection and wrong across two -- without clearing it, the second negotiation is measuring
...
async def test_a_refusal_is_re_probed_once_the_cache_expires(monkeypatch):
    """Silence is cached, but not for ever.

    A v1 device that is momentarily busy produces the same two seconds of silence as a device
    with no v1 support, and the two used to be indistinguishable for the life of the config
    entry. The cost of being wrong is asymmetric -- a plaintext device pays one extra frame per
    expiry, a misread v1 device is uncontrollable until somebody reloads it -- so the verdict
    expires. An H61F5 spent a day ignoring its owner because it did not.
    """
    monkeypatch.setattr(session_module, "V1_HANDSHAKE_TIMEOUT", 0.05)
    device = V1Device(answer=False)
    session = GoveeEncryptionSession("AA:BB:CC:DD:EE:FF")
    ...
    session.reset()
    await session.async_negotiate(device)
    assert len(device.raw_writes) == 1, "the cache should hold while it is fresh"

    monkeypatch.setattr(session_module, "V1_RULED_OUT_TTL", 0.0)
    session._v1_ruled_out_until = session_module.time.monotonic()  # what expiry looks like
    device.answer = True  # the device was there all along
    session.reset()
    await session.async_negotiate(device)
    assert len(device.raw_writes) > 1, "an expired verdict must be re-probed"
    assert session.active, "the device recovered without a config entry reload"


async def test_a_device_that_has_proven_v1_is_never_ruled_out_again(monkeypatch):
    """One completed handshake is proof of generation; a later silence is a fault, not an answer."""
    monkeypatch.setattr(session_module, "V1_HANDSHAKE_TIMEOUT", 0.05)
    device = V1Device()
    session = GoveeEncryptionSession("AA:BB:CC:DD:EE:FF")
    await device.start_notify(READ_UUID, lambda _s, data: session.decode(bytes(data)))
    ...
    device.answer = False  # the device goes quiet -- busy, or a slow notify subscription
    _reconnect(session, device)
    await session.async_negotiate(device)
    assert not session.active
    assert not session._v1_ruled_out, "a proven v1 device must never be cached as plaintext"

    device.answer = True
    _reconnect(session, device)
    await session.async_negotiate(device)
    assert session.active, "the next connection must re-probe and recover"


async def test_a_proven_v1_device_refuses_plaintext_writes(monkeypatch):
    """The half of the fix that stops the silent lie.

    Falling back to plaintext on a v1 device makes every command report success and move
    nothing. The session says so; the transport is what refuses.
    """
    monkeypatch.setattr(session_module, "V1_HANDSHAKE_TIMEOUT", 0.05)
    device = V1Device()
    session = GoveeEncryptionSession("AA:BB:CC:DD:EE:FF")
    ...
    await session.async_negotiate(device)
    assert not session.writes_would_be_ignored, "a key is in force"

    device.answer = False
    _reconnect(session, device)
    await session.async_negotiate(device)
    assert session.writes_would_be_ignored, "no key, and plaintext is known to be discarded"

    device.answer = True
    _reconnect(session, device)
    await session.async_negotiate(device)
    assert not session.writes_would_be_ignored, "recovered by reconnecting"


async def test_a_plaintext_device_never_refuses_writes(monkeypatch):
    """The refusal is scoped to proven v1 devices. Real plaintext models must be unaffected."""
    monkeypatch.setattr(session_module, "V1_HANDSHAKE_TIMEOUT", 0.05)
    device = V1Device(answer=False)
    session = GoveeEncryptionSession("AA:BB:CC:DD:EE:FF")
    await device.start_notify(READ_UUID, lambda _s, data: session.decode(bytes(data)))
    ...
    await session.async_negotiate(device)
    assert not session.active
    assert not session.writes_would_be_ignored
    packet = build_brightness(40)
    assert session.encode(packet) == packet, "plaintext devices still get plaintext"
